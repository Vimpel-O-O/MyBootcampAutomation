...
Feature: Test cases for user story #5
  ...
  Scenario Outline: Creating an appointment by administrator (Main)
    ...
    Examples:
      ...
      #With alphanumeric and special chars patient name (Pass):
      ...
      #Boundary condition testing in patient name field:
            #Pass:
      ...
      #Without specifying any date (Fail):
      ...
      #With alphanumeric and special chars in "Purpose of appointment" (pass):
      ...
      #Boundary condition testing in "Purpose of appointment" field:
            #Pass:
    ...
    #In result only Examples 1.6, 1.7, 1.8, 1.14 should fail, all others should pass
...
#Fail cases:
  ...
  Scenario Outline: Creating an appointment by administrator without choosing a specialist
    ...
    And I wait for 1 sec
    Examples:
      | envVersion | Add specialist button                          | Appointments section button                  | Add appointment button                          | Patient first and last name field | Patient name | Purpose of appointment field | Purpose    | Date         | Date field            | First available time                                                                 | Save button                           |
      | "qa2"      | "//button[contains(text(), 'Add specialist')]" | "//button[contains(text(), 'Appointments')]" | "//button[contains(text(), 'Add appointment')]" | "//input[@id='user_name']"        | "Matt Joe"   | "//textarea[@id='note']"     | "Backache" | "12/09/2023" | "//input[@id='date']" | "//button[contains( text(), 'Save')]/preceding::div[@role = 'radiogroup']/button[1]" | "//button[contains( text(), 'Save')]" |
